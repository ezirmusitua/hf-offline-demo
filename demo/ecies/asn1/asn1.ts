...
export class ASN1 {
  ...
  static Spec(tag: Tag, objs: ASN1 | ASN1[], isCompound: boolean = true): ASN1 {
    const bytes = Array.isArray(objs)
      ? Buffer.concat(objs.map(obj => obj.toDER()))
      : objs.toDER();
  ...
  private static _fromDER(bufv: BufferVisitor, deepParse: boolean): ASN1 {
    ...
    const asn1 = new ASN1(
      tagClass,
      tag,
      bufv.buf.slice(bufv.start, bufv.end),
      isCompound
    );
  ...
  validate(tpl: Template, captures: Captures = {}): Error | null {
    if (this.class !== tpl.class) {
      return new Error(
        `ASN.1 object validate failure for ${tpl.name} : error class ${
          Class[this.class]
        }`
      );
    }
    ...
    if (!tplTags.includes(this.tag)) {
      return new Error(
        `ASN.1 object validate failure for ${tpl.name}: error tag ${
          Tag[this.tag]
        }`
      );
    }
    ...
    if (Array.isArray(tpl.value)) {
      ...
      for (let i = 0, j = 0; i < tpl.value.length; i++) {
        if (values[j] != null) {
          ...
          return new Error(
            `ASN.1 object validate failure for ${tpl.value[i].name}: not exists`
          );
        }
      }
